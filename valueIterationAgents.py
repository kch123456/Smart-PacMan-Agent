...
class ValueIterationAgent(ValueEstimationAgent):
    ...
    def computeQValueFromValues(self, state, action):
        ...
        """
        "*** YOUR CODE HERE ***"
        ##compute QValue for one singlular state
        total = 0
        listOfTS = self.mdp.getTransitionStatesAndProbs(state,action)
        for ts in listOfTS:
            reward = self.mdp.getReward(state,action,ts[0])
            total += ts[1]*(reward + (self.discount*self.getValue(ts[0])))
